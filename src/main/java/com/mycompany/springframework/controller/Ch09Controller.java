...
/*	@PostMapping("/fileUpload")
	...
	public String fileUpload(Ch09FileUploadForm form) throws IOException{
		...
		form.getAttach().transferTo(destFile); // 디렉토리 없으면 새로 만드는 기능
		return "redirect:/ch09/fileUploadForm";
	}
	...
	public String downloadFileList(Model model) {
		File destDir = new File("C:/Temp/uploadFiles");
		String[] fileNames = destDir.list(); //디렉토리 안에 있는 파일 이름을 배열로 리턴
	...
	public void downloadFile(String fileName ,  
		...
		String fileType = request.getServletContext().getMimeType(fileName);	//동적으로 알아내야함 image, jpeg
		//한글로 되어있는 파일 이름 => ISO-8859-1 문자셋으로 구성된 파일 이름으로 변환
		fileName = new String(fileName.getBytes("UTF-8"), "ISO-8859-1"); // 안해주면 브라우저에서 한글 파일이 깨짐
		
		//응답 헤더에 저장할 내용
		response.setContentType(fileType);
		
		//주석 처리시 파일 다운로드가 안되고 바로 파일이 열림
